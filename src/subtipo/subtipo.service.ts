...
export class SubtipoService {
  ...
  async create(createSubtipoDto: CreateSubtipoDto): Promise<Subtipo> {
    try {
      ...
      if (!tipo) {
        throw new NotFoundException(`Tipo con ID ${createSubtipoDto.tipo_id} no encontrado`);
      }
   ...
   */
  async update(id: number, updateSubtipoDto: UpdateSubtipoDto): Promise<Subtipo> {
    ...
    if (updateSubtipoDto.tipo_id && updateSubtipoDto.tipo_id !== subtipo.tipo_id) {
      const tipo = await this.tipoRepository.findOne({
        where: { id: updateSubtipoDto.tipo_id }
      });

      if (!tipo) {
        throw new NotFoundException(`Tipo con ID ${updateSubtipoDto.tipo_id} no encontrado`);
      }
    }
